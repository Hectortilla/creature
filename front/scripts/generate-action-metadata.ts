...
function parseActionDataFromTypes(): ParsedFieldMetadata[] {
	...
	while ((match = fieldPattern.exec(typeContent)) !== null) {
		...
		const descriptionLines = jsdoc
			...
			.filter(
				(l) => l && !l.startsWith("/") && !l.startsWith("*") && l.length > 0,
			);
		...
		if (!mainDescription && descriptionLines.length > 0) {
			mainDescription =
				descriptionLines.find((l) => l && !l.match(/^[A-Z]/)) ||
				descriptionLines[0] ||
				"";
		}
		...
		if (tsType.includes("\n") || tsType.endsWith("{")) {
			...
			let braceCount =
				(tsType.match(/{/g) || []).length - (tsType.match(/}/g) || []).length;
	...
	try {
		...
		try {
			...
			try {
				...
				console.warn(
					"   Make sure the backend server is running or update NO_FIELD_ACTION_TYPES manually.",
				);
			}
			...
			if (openApiContent) {
				...
				if (actionDataSchema?.properties?.action_type?.examples) {
					...
					if (Array.isArray(examples)) {
						examples.forEach((actionType: string) =>
			...
			console.warn(
				"Could not parse OpenAPI JSON for action type examples:",
				error,
			);
		}
