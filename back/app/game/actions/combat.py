...
from app.models.game.enums import Zone, TurnPhase, StatusType
...
def _damage_and_destruction(
    ...
    events: list[GameEvent] = [DamageDealtEvent(
        game_id=state.game_id, source_id=source.instance_id, target_id=target.instance_id,
        damage_type=attack.type, base_damage=base_damage, element_bonus=element_bonus,
        defense_reduction=defense_reduction, final_damage=amount,
    ...
    if target.current_health - amount <= 0:
        events.append(CardDestroyedEvent(
            ...
            owner_id=target.owner_id, card_name=target.name, destroyed_by=source.instance_id,
        ))
    return events
...
def build_combat_events(
    ...
    if damage_calc.final_damage > 0:
        events.extend(_damage_and_destruction(
            state, attacker, target, attack, damage_calc.final_damage,
    ...
    events.append(AttackResolvedEvent(
...
class AttackAction(Action):
    ...
    def validate(self, state: "GameState") -> "ValidationResult":
        ...
        if blocked:
            return ValidationResult(valid=False, error="Card cannot attack due to status", error_code="ATTACK_BLOCKED_BY_STATUS")
        ...
        if cost_atom:
            ...
            if self.cost_card_id not in candidates:
                return ValidationResult(valid=False, error="This attack requires a valid graveyard card cost", error_code="MISSING_ATTACK_COST")
        ...
        if splash_atom:
            secondary_options = {card.instance_id for card in _get_adjacent_targets(state, opponent, self.target_card_id)}
            if len(secondary_options) > 1 and self.secondary_target_card_id not in secondary_options:
                return ValidationResult(valid=False, error="Secondary splash target required", error_code="MISSING_SECONDARY_TARGET")
            if self.secondary_target_card_id and self.secondary_target_card_id not in secondary_options:
                return ValidationResult(valid=False, error="Invalid secondary target", error_code="INVALID_SECONDARY_TARGET")
        return ValidationResult(valid=True)
    ...
    def to_events(self, state: "GameState") -> list[GameEvent]:
        ...
        if cost_atom and self.cost_card_id:
            ...
            if cost_card:
                prefix_events.append(CardExiledEvent(
                    game_id=state.game_id,
        ...
        if len(opponent.zones[Zone.ATTACKING.name].card_ids) == 0:
            if len(opponent.zones[Zone.SUPPORTING.name].card_ids) > 0:
                return prefix_events + [NoDefenderEvent(
        ...
        for idx, tgt in enumerate(targets):
            events.extend(build_combat_events(
                state,
                attacker,
                tgt,
                attack,
    ...
    def _dice_lock_events(self, attacker: "GameCard", state: "GameState") -> tuple[list[GameEvent], bool]:
        ...
        events: list[GameEvent] = [
            DiceRolledEvent(game_id=state.game_id, roller_id=attacker.owner_id, faces=6, result=result, purpose="dice_locked_attack")
        ]
        if result == required:
            events.append(StatusExpiredEvent(game_id=state.game_id, target_id=attacker.instance_id, status_id=status.status_id))
            return events, True
        return events, False
        ...
        for attacker_id in player.zones[Zone.ATTACKING.name].card_ids:
            ...
            for attack in attacker.attacks:
                ...
                for target_id in opponent.zones[Zone.ATTACKING.name].card_ids:
                    secondary_options = _get_adjacent_targets(state, opponent, target_id) if get_splash_atom(attacker, attack.attack_id) else []
                    secondary_ids = [card.instance_id for card in secondary_options] or [""]
                    for cost_id in cost_ids:
                        for secondary_id in secondary_ids:
                            actions.append(cls(player_id=player_id, attacker_id=attacker_id, attack_id=attack.attack_id, target_card_id=target_id, secondary_target_card_id=secondary_id, cost_card_id=cost_id))
                if len(opponent.zones[Zone.ATTACKING.name].card_ids) == 0:
                    for cost_id in cost_ids:
                        actions.append(cls(player_id=player_id, attacker_id=attacker_id, attack_id=attack.attack_id, target_card_id="", cost_card_id=cost_id))
        return actions
...
class ResolveForcedSwapAction(Action):
    ...
    def validate(self, state: "GameState") -> "ValidationResult":
        ...
        if not target_id or target_id not in player.zones[Zone.ATTACKING.name].card_ids:
            return ValidationResult(valid=False, error="Forced swap target is invalid", error_code="INVALID_FORCED_SWAP_TARGET")
        return ValidationResult(valid=True)
...
class ReviveFromGraveyardAction(Action):
    ...
    def validate(self, state: "GameState") -> "ValidationResult":
        ...
        if not can_revive_from_graveyard(state, source):
            return ValidationResult(valid=False, error="Card cannot revive from graveyard", error_code="REVIVE_NOT_ALLOWED")
            ...
            if source and can_revive_from_graveyard(state, source):
                for graveyard_id in graveyard_ids:
                    if state.get_card(graveyard_id):
                        actions.append(cls(player_id=player_id, source_card_id=source_id, graveyard_card_id=graveyard_id))
        return actions
