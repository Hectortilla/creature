...
from app.models.game.events import CardEvolvedEvent, GameEvent
    ...
    def validate(self, state: GameState) -> ValidationResult:
        ...
        if state.is_first_turn(self.player_id):
            return ValidationResult(
                valid=False, error="Cannot evolve cards on first turn", error_code="FIRST_TURN_RESTRICTION"
            )
        if state.is_second_turn(self.player_id):
            return ValidationResult(
                valid=False, error="Cannot evolve cards on second turn", error_code="SECOND_TURN_RESTRICTION"
            )
        ...
        if target_card.card_id != evo_card.evolves_from_id:
            return ValidationResult(
                valid=False, error="Target card does not match evolution requirement", error_code="EVOLUTION_MISMATCH"
            )
    ...
    def to_events(self, state: GameState) -> list[GameEvent]:
        ...
        return [
            CardEvolvedEvent(
                ...
                base_card_id=self.target_card_id,
