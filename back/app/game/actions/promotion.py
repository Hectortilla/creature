...
from app.models.game.events import GameEvent, CardPromotedEvent
...
class PromoteAction(Action):
    ...
    def validate(self, state: "GameState") -> "ValidationResult":
        ...
        if player.zones[Zone.ATTACKING.name].is_full:
            return ValidationResult(valid=False, error="Attacking zone is full (max 2 cards)", error_code="ATTACKING_ZONE_FULL")
        card = state.get_card(self.instance_id)
        if not card or not card.can_promote:
            return ValidationResult(valid=False, error="Card must spend at least one full turn in supporting zone before promotion", error_code="CARD_NOT_READY")
    ...
    def get_valid(cls, state: "GameState", player_id: str) -> list[Action]:
        ...
        return [
            ...
            if state.get_card(cid) and state.get_card(cid).can_promote
        ]
