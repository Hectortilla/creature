...
from app.models.game.events import CardAssociatedEvent, GameEvent
    ...
    def validate(self, state: GameState) -> ValidationResult:
        ...
        if state.is_first_turn(self.player_id):
            return ValidationResult(
                valid=False, error="Cannot associate cards on first turn", error_code="FIRST_TURN_RESTRICTION"
            )
        if self.association_card_id == self.target_card_id:
            return ValidationResult(
                valid=False, error="Cannot associate a card with itself", error_code="SELF_ASSOCIATION"
            )
        ...
        if self.association_card_id not in supporting.card_ids and not (
            direct_from_hand and self.association_card_id in hand.card_ids
        ):
            return ValidationResult(
                valid=False,
        ...
        if self.target_card_id not in supporting.card_ids and self.target_card_id not in attacking.card_ids:
            return ValidationResult(
                valid=False, error="Target card must be in an active zone", error_code="INVALID_TARGET"
            )
        if not assoc_card or not assoc_card.association_ids:
            return ValidationResult(
                valid=False, error="Card cannot be used as an association", error_code="NOT_ASSOCIATION_CARD"
            )
        ...
        if not associations_allowed(state, target_card):
            return ValidationResult(
                valid=False, error="Associations are forbidden on this card", error_code="ASSOCIATIONS_FORBIDDEN"
            )
        if len(target_card.associations) >= get_association_limit(state, target_card):
            return ValidationResult(
                valid=False, error="Association limit reached", error_code="ASSOCIATION_LIMIT_REACHED"
            )
        ...
        if self.swap_with_supporting_card_id:
            if target_card.zone != Zone.ATTACKING:
                return ValidationResult(
                    valid=False,
                    error="Swap association target must be in attacking zone",
                    error_code="INVALID_SWAP_ASSOCIATION_TARGET",
                )
            if self.swap_with_supporting_card_id not in supporting.card_ids:
                return ValidationResult(
                    valid=False, error="Swap card must be in supporting zone", error_code="INVALID_SWAP_CARD"
                )
    ...
    def get_valid(cls, state: GameState, player_id: str) -> list[Action]:
        ...
        for assoc_id in sources:
            ...
            if assoc_card and assoc_card.association_ids:
                if assoc_id in player.zones[Zone.HAND.name].card_ids and not association_allows_direct_from_hand(
                    assoc_card
                ):
                    continue
                    ...
                    for swap_id in swap_ids:
                        actions.append(
                            cls(
                                ...
                                swap_with_supporting_card_id=swap_id,
                            )
                        )
        return actions
