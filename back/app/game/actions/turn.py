...
class DrawAction(Action):
    ...
    def validate(self, state: "GameState") -> "ValidationResult":
        ...
        if len(deck.card_ids) < self.count:
            return ValidationResult(valid=False, error=f"Not enough cards in deck (have {len(deck.card_ids)}, need {self.count})", error_code="NOT_ENOUGH_CARDS")
...
class PassPhaseAction(Action):
    ...
    def to_events(self, state: "GameState") -> list[GameEvent]:
        ...
        if next_phase:
            ...
            events.append(TurnEndedEvent(game_id=state.game_id, player_id=self.player_id, turn_number=state.turn_number))
            events.append(ElementsRestoredEvent(game_id=state.game_id, player_id=next_player_id, elements=dict(next_player.element_pool.max_elements)))
            events.append(TurnStartedEvent(game_id=state.game_id, player_id=next_player_id, turn_number=state.turn_number + 1, is_first_turn=next_player.turn_count == 0))
            events.extend(_draw_events(state, next_player_id, next_player))
            events.append(PhaseChangedEvent(game_id=state.game_id, player_id=next_player_id, from_phase=TurnPhase.DRAW, to_phase=TurnPhase.PLACEMENT))
...
class ConcedeAction(Action):
    ...
    def to_events(self, state: "GameState") -> list[GameEvent]:
        ...
        return [GameEndedEvent(
            game_id=state.game_id, winner_id=opponent.player_id,
            loser_id=self.player_id, reason="Player conceded",
        )]


# ── Helpers ─────────────────────────────────────────────────────────────

def _draw_events(state: "GameState", player_id: str, player, count: int = None) -> list[GameEvent]:
    if count is None:
    ...
    for i in range(min(count, len(deck.card_ids))):
        ...
        events.append(CardDrawnEvent(
            game_id=state.game_id, player_id=player_id,
            instance_id=instance_id, card_id=card.card_id if card else 0,
...
def _should_skip_phase(state: "GameState", phase: TurnPhase, player_id: str) -> bool:
    ...
    if phase == TurnPhase.PROMOTION:
        if player.zones[Zone.ATTACKING.name].is_full:
            return True
        return not any(
            state.get_card(cid) and state.get_card(cid).can_promote
    ...
    if phase == TurnPhase.EVOLUTION:
        ...
        evolvable_base_ids = {
            c.card_id
            for cid in player.get_active_cards()
            if (c := state.get_card(cid)) and c.can_evolve
        }
        return not any(
            (c := state.get_card(cid))
            and c.evolves_from_id is not None
            and c.evolves_from_id in evolvable_base_ids
