...
from app.models.game.events import CardPlayedEvent, GameEvent
...
class PlayCardAction(Action):
    ...
    def validate(self, state: GameState) -> ValidationResult:
        ...
        if len(self.instance_ids) > available:
            return ValidationResult(
                valid=False,
                error=f"Not enough slots (have {available}, need {len(self.instance_ids)})",
    ...
    def _validate_card(state: GameState, hand, instance_id: str) -> ValidationResult:
        ...
        if instance_id not in hand.card_ids:
            return ValidationResult(
                valid=False, error=f"Card {instance_id} is not in your hand", error_code="CARD_NOT_IN_HAND"
            )
        card = state.get_card(instance_id)
        if card and card.evolves_from_id is not None:
            return ValidationResult(
                valid=False,
                error=f"Evolution card {instance_id} cannot be placed directly; use the Evolution Phase",
                error_code="EVOLUTION_CARD_PLACEMENT",
            )
        return ValidationResult(valid=True)
