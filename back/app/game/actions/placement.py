...
class PlayCardAction(Action):
    ...
    def validate(self, state: "GameState") -> "ValidationResult":
        ...
        if self.instance_id not in player.zones[Zone.HAND.name].card_ids:
            return ValidationResult(valid=False, error="Card is not in your hand", error_code="CARD_NOT_IN_HAND")
        if player.zones[Zone.SUPPORTING.name].is_full:
            return ValidationResult(valid=False, error="Supporting zone is full (max 3 cards)", error_code="SUPPORTING_ZONE_FULL")
...
class MultiPlayCardAction(Action):
    ...
    def validate(self, state: "GameState") -> "ValidationResult":
        ...
        for cid in self.instance_ids:
            if cid not in hand.card_ids:
                return ValidationResult(valid=False, error=f"Card {cid} is not in your hand", error_code="CARD_NOT_IN_HAND")
