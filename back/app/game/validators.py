...
from app.game.actions import Action, ConcedeAction, ForceDefendAction, ResolveForcedSwapAction
...
class RuleValidator:
    ...
    def validate(self, state: GameState, action: Action) -> ValidationResult:
        ...
        if state.status != GameStatus.IN_PROGRESS:
            if state.status == GameStatus.PAUSED and not isinstance(
                action, (ForceDefendAction, ResolveForcedSwapAction, ConcedeAction)
            ):
                return ValidationResult(
                    valid=False, error="Game is paused, waiting for forced defend action", error_code="GAME_PAUSED"
                )
        ...
        if (
            not isinstance(action, (ForceDefendAction, ResolveForcedSwapAction, ConcedeAction))
        ...
        # Phase check
        if action.valid_phases is not None and state.current_phase not in action.valid_phases:
            return ValidationResult(
                valid=False,
