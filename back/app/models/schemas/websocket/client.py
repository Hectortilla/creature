...
class ActionData(BaseModel):
    ...
    instance_id: str | None = Field(
        default=None, description="Card instance ID (used by: promote, force_defend)", examples=["card_instance_123"]
    )
    ...
    count: int = Field(default=1, description="Number of cards to draw (used by: draw)", examples=[1, 2, 3])
    target_card_id: str | None = Field(
        default=None,
        description="Target card instance ID (used by: associate, evolve, attack)",
        examples=["card_instance_789"],
    )
    attacker_id: str | None = Field(
        default=None, description="Attacker card instance ID (used by: attack)", examples=["card_instance_123"]
    )
    attack_id: int | None = Field(default=None, description="Attack ID to use (used by: attack)", examples=[1, 2])
    secondary_target_card_id: str | None = Field(
        default=None,
        description="Secondary target card instance ID (used by: splash attack effects)",
        examples=["card_instance_456"],
    )
    cost_card_id: str | None = Field(
        default=None, description="Card instance ID paid as a special attack cost", examples=["graveyard_card_123"]
    )
    supporting_card_id: str | None = Field(
        default=None, description="Supporting card instance ID (used by: swap)", examples=["card_instance_123"]
    )
    attacking_card_id: str | None = Field(
        default=None, description="Attacking card instance ID (used by: swap)", examples=["card_instance_456"]
    )
    ...
    association_card_id: str | None = Field(
        default=None, description="Association card instance ID (used by: associate)", examples=["card_instance_123"]
    )
    swap_with_supporting_card_id: str | None = Field(
        default=None,
        description="Supporting card selected for association-driven free swap",
        examples=["card_instance_456"],
    )
    evolution_card_id: str | None = Field(
        default=None, description="Evolution card instance ID (used by: evolve)", examples=["card_instance_123"]
    )
