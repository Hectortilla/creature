...
class PhaseChangedEvent(GameEvent):
    ...
    player_id: str = ""
    from_phase: TurnPhase | None = None
...
# Discriminated union for OpenAPI schema generation. Annotated as ``Any`` because
# membership is built dynamically from a tuple of event classes, which mypy cannot
# evaluate as a static type alias.
GameEventUnion: Any = Annotated[
    Union[tuple(_ALL_EVENT_CLASSES)],  # noqa: UP007 - dynamic union from a tuple of types
    Field(discriminator="event_type"),
]

# Event type registry for deserialization
EVENT_TYPES: dict[str, type[GameEvent]] = {cls.__name__: cls for cls in _ALL_EVENT_CLASSES}
