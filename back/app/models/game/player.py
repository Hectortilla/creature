...
class PlayerState(GameBaseModel):
    ...
    deck: list[GameCardInput] | None = Field(default=None, exclude=True)
    ...
    def initialize_zones(self) -> PlayerState:
        ...
        if not self.zones or len(self.zones) == 0:
            object.__setattr__(
                self, "zones", {zone.name: ZoneState(zone=zone, owner_id=self.player_id) for zone in Zone}
            )
        for zone in Zone:
    ...
    def get_active_cards(self) -> list[str]:
        ...
        return self.zones[Zone.SUPPORTING.name].card_ids + self.zones[Zone.ATTACKING.name].card_ids
