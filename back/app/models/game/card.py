...
from app.models.game.enums import CardStatus, StatusType, Zone
...
class GameCard(CardIdentityFields, CardCombatFields, GameBaseModel):
    ...
    def can_promote(self) -> bool:
        ...
        return self.zone == Zone.SUPPORTING and self.turns_in_zone >= 1 and self.status != CardStatus.ASSOCIATED
    ...
    @classmethod
    def create(
        cls, card_id: int, owner_id: str, name: str, health: int, physical_defence: int, magic_defence: int, **kwargs
    ) -> GameCard:
...
class GameCardInput(GameBaseModel):
        ...
        for force in necessary_force:
            if isinstance(force, dict):
                ...
                if "elementData" in force and "value" in force:
                    ...
                    normalized.append({"element_id": force["elementData"]["id"], "amount": force["value"]})
    ...
    def from_card_read(cls, card: CardReadWithRelations) -> GameCardInput:
        ...
        return cls(
            ...
            effect_specs=[EffectSpec.model_validate(effect.model_dump()) for effect in getattr(card, "effects", [])],
            evolves_from_id=card.is_evolution_id,
        )


__all__ = ["ActiveStatus", "AttackInput", "EffectSpec", "GameCard", "GameCardInput"]
