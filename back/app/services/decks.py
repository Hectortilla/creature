...
class DeckService(BaseService[Deck, DeckCreate]):
    ...
    def get_user_deck(self, deck_id: int) -> Deck | None:
        ...
        deck = self.db.exec(select(Deck).where(Deck.id == deck_id, Deck.user_id == self.user_id)).first()
        return deck

    def create(self, data: DeckCreate) -> Deck:
        """Create a new deck for the current user."""
        deck = Deck(**data.model_dump(), user_id=self.user_id)
        self.db.add(deck)
        self.db.commit()
        self.db.refresh(deck)
        return deck

    def update(self, deck_id: int, data: DeckUpdate) -> Deck | None:
    ...
    def delete(self, deck_id: int) -> bool:
        ...
        self.db.exec(select(DeckCard).where(DeckCard.deck_id == deck_id))
        deck_cards = self.db.exec(select(DeckCard).where(DeckCard.deck_id == deck_id)).all()
    ...
    def add_card_to_deck(self, deck_id: int, card_id: int, position: int | None = None) -> bool:
        ...
        card = self.db.exec(select(Card).where(Card.id == card_id)).first()
        if not card:
        ...
        if current_count >= self.deck_size:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Deck is full. Maximum {self.deck_size} cards allowed."
            )

        # Add card to deck
        deck_card = DeckCard(deck_id=deck_id, card_id=card_id, position=position)
    ...
    def get_enriched(self, deck_id: int) -> DeckReadWithCards | None:
