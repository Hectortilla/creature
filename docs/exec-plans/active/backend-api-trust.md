...
- [x] **Status:** ✅ done — 2026-06-27 — `tests/integration/test_router_{cards,attacks,crud}.py` (10 tests): list/get-by-code/handle/name + `by-attack`/`by-ability`/`by-association` lookups, create→201 + delete→200 + delete-missing→404 + get-unknown→404, and missing-header→401 per router (crud via the elements instance, proving the factory enforces auth) — branch `spec/backend-api-trust/step-5/router-cards-attacks-crud` — PR https://app.graphite.com/github/pr/Hectortilla/creature/29
- **Notes for next agent:**
  - **Card/Attack create needs only `{code, name}`** (handle is auto-generated by `BaseService.create` via `has_handle`); element create needs only `{label}`. Seed reference rows with high codes (`9_0xx_xxx`) to dodge the pre-seeded `cards`/`attacks` unique-code rows (same trap as Step 4).
  - **`GET /cards/{value}` returns a LIST** (`get_enriched`), `GET /attacks/{value}` returns a single object — assert accordingly. Lookup by digit → code; else → handle/name (ilike).
  - **Delete returns 200** (`{"message": ...}`) for cards/attacks/crud, not 204 (decks delete was 204 — different routers). Missing/unknown → 404.
  - Step 6 (services) can reuse the `codes()`/seed-via-`session` pattern for `CardService`/`AttackService` enrichment tests.
